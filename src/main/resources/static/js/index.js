...
var menuItem = Vue.extend({
    ...
    template:[
        ...
        '	<a v-if="item.menuType == 1 && item.parentId == 0" :href="\'#\'+item.url">',
        '		<i v-if="item.icon != null" :class="item.icon"></i>',
        '		<span>{{item.menuName}}</span>',
        '	</a>',

        '	<a v-if="item.menuType == 1 && item.parentId != 0" :href="\'#\'+item.url"><i v-if="item.icon != null" :class="item.icon"></i><i v-else class="fa fa-circle-o"></i> {{item.menuName}}</a>',
        '</li>'
    ].join('')
});
